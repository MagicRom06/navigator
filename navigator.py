...
class NavigateurFichiers(urwid.WidgetPlaceholder):
    ...
    def creer_contenu(self):
        ...
        for fichier in self.liste_fichiers:
            boutton = urwid.Button(fichier)
            if os.path.isdir(os.path.join(self.chemin_courant, fichier)):
                urwid.connect_signal(boutton, 'click', self.maj_contenu, fichier)
                contenu.append(urwid.AttrMap(boutton, 'rep', focus_map='rep_focus'))
            else:
                contenu.append(urwid.AttrMap(boutton, None, focus_map='fichier'))
        return urwid.AttrMap(urwid.LineBox(urwid.ListBox(urwid.SimpleFocusListWalker(contenu)), self.chemin_courant),
                             'contenu')
    ...
    def palette(self):
        return [('contenu', 'light gray', 'light blue'), ('fichier', 'light gray', 'dark red'),
                ('rep_focus', 'white', 'dark red'), ('rep', 'black', 'light blue')]


def main():
    nav = NavigateurFichiers(os.getcwd())
    main = urwid.MainLoop(nav, nav.palette(), unhandled_input=nav.quitter)
    main.run()
